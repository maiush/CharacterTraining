...
openrlhf.cli.train_sft \
    --save_path /workspace/models/llama-3.1-8b-blend-lora \
    --eval_steps 50 \
    --max_ckpt_num 1 \
    ...
    --wandb_run_name llama-3.1-8b-blend-lora
EOF
...
if [ $? -eq 0 ]; then
    ...
    python lora_combiner.py \
        ...
        --lora_path /workspace/models/llama-3.1-8b-blend-lora \
        --output_path /workspace/models/llama-3.1-8b-generator-snapshot
    # upload model
    cd /workspace/CharacterTraining/tools
    python upload_model.py --model llama-3.1-8b-generator-snapshot --name llama-3.1-8b-generator-snapshot-0603
    # upload lora
    python upload_model.py --model llama-3.1-8b-blend-lora --name llama-3.1-8b-blend-lora-0603
fi